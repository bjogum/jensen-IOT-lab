...
## API-systemet
Diagrammet visar den övergripande arkitekturen för IoT-plattformen. Tre simulerade sensorer skickar mätdata till API:t via `HTTP POST`. API:t validerar datan och lagrar mätningarna permanent i PostgreSQL. Den senaste mätningen per sensor sparas även i Redis som cache.
